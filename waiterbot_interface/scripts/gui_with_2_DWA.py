...
class MapImage(TouchRippleBehavior, Image):
    ...
    def on_touch_down(self, touch):
        if self.collide_point(*touch.pos):
            ...
            # Reset jika sudah ada 2 titik (Klik ke-3)
            if len(self.markers) >= 2:
                ...
                # (Catatan: Input text dibiarkan agar memorinya tidak terhapus)

            # Tentukan Nomor Label (1 atau 2)
            goal_number = len(self.markers) + 1

            new_marker = Label(text=str(goal_number), font_size='30sp', color=(1, 0, 0, 1), bold=True)
...
# =============================================================================
# 4. SCREEN NAVIGASI (TAMPILAN PETA UTAMA)
# =============================================================================
...
# =============================================================================
# 5. CLASS APLIKASI UTAMA (Main Logic)
# =============================================================================
class MainApp(App):
    PAN_STEP = 50 
...
"""
        ...
        try:
            ...
            if screen.goal_coords_queue:
                ...
                if len(screen.goal_coords_queue) == 1:
                    ...
                    screen.ids.navigation_status_label.text = f"Goal 1: ({g1[0]:.2f}, {g1[1]:.2f}) | Goal 2: ({g2[0]:.2f}, {g2[1]:.2f})"
                    
        except ValueError:
            screen.ids.navigation_status_label.text = "Error: Masukkan angka yang valid!"

    def _place_marker_from_ros_coords(self, screen, map_viewer, map_x, map_y, label_text):
        if not hasattr(map_viewer, 'markers'):
            map_viewer.markers = []
    ...
    # ==========================================
    # LOGIKA PENGGANTIAN DWA DYNAMIC RECONFIGURE
    # ==========================================
    def set_dwa_params(self, goal_type):
        def _run_reconfigure():
            try:
                if goal_type == 1:
                    # Parameter cepat untuk Goal 1
                    params = "{'max_vel_x': 1.0, 'min_vel_x': -1.0, 'max_vel_trans': 1.0, 'min_vel_trans': 0.11, 'max_vel_theta': 2.75, 'min_vel_theta': 1.37, 'acc_lim_x': 10.0, 'acc_lim_theta': 2.4}"
                    print("Mengubah DWA ke profil GOAL 1 (Cepat)")
                else:
                    # Parameter lambat untuk Goal 2
                    params = "{'max_vel_x': 0.22, 'min_vel_x': -0.22, 'max_vel_trans': 0.22, 'min_vel_trans': 0.11, 'max_vel_theta': 2.4, 'min_vel_theta': 1.37, 'acc_lim_x': 2.5, 'acc_lim_theta': 2.4}"
                    print("Mengubah DWA ke profil GOAL 2 (Lambat)")

                # Memanggil command ROS secara dinamis (background process)
                cmd = f"rosrun dynamic_reconfigure dynparam set /move_base/DWAPlannerROS \"{params}\""
                subprocess.Popen(cmd, shell=True)
            except Exception as e:
                print(f"Gagal mengubah parameter DWA: {e}")

        # Jalankan di Thread agar Kivy/GUI tidak macet (freeze)
        threading.Thread(target=_run_reconfigure, daemon=True).start()
    ...
    def confirm_navigation_goal(self):
        ...
        if screen.goal_coords_queue:
            # SET PARAMETER DWA KE GOAL 1 (CEPAT)
            self.set_dwa_params(goal_type=1)
            ...
            if success:
                ...
                screen.ids.navigation_status_label.text = f"Status: Menuju Goal {self.current_goal_index}..."
        ...
        if current_pose:
            ...
            if distance < 0.20: 
                ...
                if screen.goal_coords_queue:
                    ...
                    # SET PARAMETER DWA KE GOAL 2 (LAMBAT)
                    self.set_dwa_params(goal_type=2)
